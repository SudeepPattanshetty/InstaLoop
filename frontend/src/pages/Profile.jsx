...
const Profile = () => {
  ...
  return user ? (
    <div className='relative h-full overflow-y-scroll bg-gray-50 p-6'>
        ...
        <div className='mt-6'>
          <div className='bg-white rounded-xl shadow p-1 flex max-w-md mx-auto'>
            {["posts", "media", "likes"].map((tab) => (
              <button onClick={() => setActiveTab(tab)} className={`flex-1 px-4 py-2 text-sm font-medium rounded-lg transition-colors cursor-pointer ${activeTab === tab ? "bg-indigo-600 text-white" : "text-gray-600 hover:text-gray-900"}`}>
      ...
      {/* Edit Profile Modal */}
      {showEdit && <ProfileModal showEdit /> }
